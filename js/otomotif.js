document.addEventListener('DOMContentLoaded', () => {
  ...
  const container = document.querySelector('.otomotif');
  if (!container) return;
  ...
  async function initCategory() {
    const res = await fetch('https://lampost.co/wp-json/wp/v2/categories?slug=otomotif');
    const data = await res.json();
  ...
  async function loadPosts() {
    ...
    try {
      const res = await fetch(`https://lampost.co/wp-json/wp/v2/posts?categories=${categoryId}&per_page=${PER_PAGE}&page=${page}&orderby=date&order=desc`);
  ...
  initCategory().then(ok => {
    if (ok) {
      loadPosts();
      loadMoreBtn.addEventListener('click', loadPosts);
    } else {
      container.innerHTML = '<p>Kategori otomotif tidak ditemukan</p>';
    }
  });
